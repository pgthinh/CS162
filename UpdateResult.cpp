...
void ChangeMarkFileAfterUpdate(Mark& changeMark, string& CourseID, string& studentID) {
	...
	while (course_cur) {
		...
		else  fout << course_cur->CourseID << " " << mark_cur->totalMark << " " << mark_cur->finalMark << " " << mark_cur->midtermMark << " " << mark_cur->otherMark << endl;
		course_cur = course_cur->next_Course;
		mark_cur = mark_cur->next_Mark;
	}
	...
	fin.open(path); // fin >> student_cur->ID >> mark_cur->totalMark >> mark_cur->finalMark >> mark_cur->midtermMark >> mark_cur->otherMark >> student_cur->Name
	...
	fout.open(path);
	while (student_cur) {
		if (!student_cur->next_Student) fout << student_cur->ID << " " << mark_cur->totalMark << " " << mark_cur->finalMark << " " << mark_cur->midtermMark << " " << mark_cur->otherMark << " " << student_cur->Name;
		else fout << student_cur->ID << " " << mark_cur->totalMark << " " << mark_cur->finalMark << " " << mark_cur->midtermMark << " " << mark_cur->otherMark << " " << student_cur->Name << endl;
		student_cur = student_cur->next_Student;
		mark_cur = mark_cur->next_Mark;
	}
